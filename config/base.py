...
def get_config():
    ...
    config.run_name = "Fsy"
    config.debug = True 
    # 是否执行 eval_fn。默认 False：只保存 checkpoint，不跑 eval（省去测试集采样 + judge 打分）。
    # 需要周期性评估时再改为 True。
    ...
    # DAPO 式动态采样：是否丢弃零方差组。组内 reward 全相同 -> advantage 恒为 0，
    # 这些组不提供梯度信号却占用 backward 预算，整组剔除。
    train.drop_zero_std_groups = False
